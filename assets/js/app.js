...
function renderWelcomeText() {
  ...
  elements.welcomeText.textContent = `${languagePack.signedInAs} ${state.currentUser.name} (${roleLabel}) · ${todayLabel}`;
}
...
function renderBudgetSummary(monthTotal) {
  ...
  elements.budgetAmount.value = String(budget);
...
function renderReportPieChart(expenses) {
  ...
  const width = elements.reportPieChart.width;
  const height = elements.reportPieChart.height;
  ...
  rows.forEach((row, index) => {
    const arcSize = (row.total / totalAmount) * Math.PI * 2;
    chartContext.beginPath();
    chartContext.moveTo(cx, cy);
    chartContext.arc(cx, cy, radius, cursorAngle, cursorAngle + arcSize);
    chartContext.closePath();
    chartContext.fillStyle = colorPalette[index % colorPalette.length];
    chartContext.fill();
    cursorAngle += arcSize;
  });

  // Create a donut center for readability and draw total at center.
  chartContext.beginPath();
  chartContext.arc(cx, cy, innerRadius, 0, Math.PI * 2);
  chartContext.fillStyle = "#12202a";
  chartContext.fill();

  chartContext.fillStyle = "#eaf3f0";
  chartContext.font = "600 14px 'Plus Jakarta Sans', sans-serif";
  chartContext.textAlign = "center";
  chartContext.textBaseline = "middle";
  chartContext.fillText(dynamicText.pieTotalLabel, cx, cy - 10);
  chartContext.font = "700 12px 'Plus Jakarta Sans', sans-serif";
  chartContext.fillText(formatMoney(totalAmount), cx, cy + 12);

  const legendRows = rows
    .map((row, index) => {
      const percent = Math.round((row.total / totalAmount) * 100);
      const color = colorPalette[index % colorPalette.length];

      return `<li>
        <span class="pie-swatch" style="background:${color}"></span>
        <span class="pie-label">${escapeHtml(row.category)}</span>
        <span class="pie-value">${percent}% · ${formatMoney(row.total)}</span>
      </li>`;
    })
    .join("");
...
function renderSettingsView() {
  ...
  if (elements.settingProfileName) {
    elements.settingProfileName.value = state.currentUser.name || "";
  }
  if (elements.settingProfileEmail) {
    elements.settingProfileEmail.value = state.currentUser.email || "";
  }
  ...
  updateProfilePicturePreview(state.currentUser.profilePictureDataUrl || "", state.currentUser.profilePictureName || "");
...
function handlePreferencesSettingsSubmit(event) {
  ...
  const settings = {
    currency: String(elements.settingCurrency ? elements.settingCurrency.value : DEFAULT_USER_SETTINGS.currency),
    language: String(elements.settingLanguage ? elements.settingLanguage.value : DEFAULT_USER_SETTINGS.language),
    dateLocale: String(elements.settingDateFormat ? elements.settingDateFormat.value : DEFAULT_USER_SETTINGS.dateLocale),
    timezone: String(elements.settingTimezone ? elements.settingTimezone.value : DEFAULT_USER_SETTINGS.timezone),
    weekStart: String(elements.settingWeekStart ? elements.settingWeekStart.value : DEFAULT_USER_SETTINGS.weekStart),
    defaultRange: String(elements.settingDefaultRange ? elements.settingDefaultRange.value : DEFAULT_USER_SETTINGS.defaultRange)
  };
  ...
  if (!SUPPORTED_DATE_LOCALES.includes(settings.dateLocale)) {
    settings.dateLocale = DEFAULT_USER_SETTINGS.dateLocale;
  }

  if (!SUPPORTED_TIMEZONES.includes(settings.timezone)) {
    settings.timezone = DEFAULT_USER_SETTINGS.timezone;
  }

  if (!SUPPORTED_WEEK_STARTS.includes(settings.weekStart)) {
    settings.weekStart = DEFAULT_USER_SETTINGS.weekStart;
  }

  if (!isSupportedDateRange(settings.defaultRange)) {
    settings.defaultRange = DEFAULT_USER_SETTINGS.defaultRange;
  }
...
function handleAppearanceSettingsSubmit(event) {
  ...
  const appearance = {
    themeMode: String(elements.settingThemeMode ? elements.settingThemeMode.value : DEFAULT_APPEARANCE_SETTINGS.themeMode),
    themeColor: String(elements.settingThemeColor ? elements.settingThemeColor.value : DEFAULT_APPEARANCE_SETTINGS.themeColor),
    fontSize: String(elements.settingFontSize ? elements.settingFontSize.value : DEFAULT_APPEARANCE_SETTINGS.fontSize),
    widgets: {
...
function getCurrentUserAppearanceSettings() {
  ...
  return {
    themeMode: ["dark", "light"].includes(configured.themeMode)
      ? configured.themeMode
      : DEFAULT_APPEARANCE_SETTINGS.themeMode,
    themeColor: SUPPORTED_THEME_COLORS.includes(configured.themeColor)
      ? configured.themeColor
      : DEFAULT_APPEARANCE_SETTINGS.themeColor,
    fontSize: ["small", "medium", "large"].includes(configured.fontSize)
      ? configured.fontSize
      : DEFAULT_APPEARANCE_SETTINGS.fontSize,
    widgets: {
      insights: configured.widgets && typeof configured.widgets.insights === "boolean"
        ? configured.widgets.insights
        : DEFAULT_APPEARANCE_SETTINGS.widgets.insights,
      forecast: configured.widgets && typeof configured.widgets.forecast === "boolean"
        ? configured.widgets.forecast
        : DEFAULT_APPEARANCE_SETTINGS.widgets.forecast,
      recent: configured.widgets && typeof configured.widgets.recent === "boolean"
        ? configured.widgets.recent
        : DEFAULT_APPEARANCE_SETTINGS.widgets.recent,
      categories: configured.widgets && typeof configured.widgets.categories === "boolean"
        ? configured.widgets.categories
...
function getCurrentUserSettings() {
  ...
  const configured = state.userPreferences[state.currentUser.id];
  if (!isPlainObject(configured)) {
    return {
      ...DEFAULT_USER_SETTINGS,
      privacy: { ...DEFAULT_PRIVACY_SETTINGS }
  ...
  const configuredWeekStart = String(configured.weekStart || "").trim().toLowerCase();
  const safeWeekStart = SUPPORTED_WEEK_STARTS.includes(configuredWeekStart)
    ? configuredWeekStart
    : DEFAULT_USER_SETTINGS.weekStart;
  const configuredCurrency = String(configured.currency || "").trim().toUpperCase();
  const safeCurrency = SUPPORTED_CURRENCIES.includes(configuredCurrency)
    ? configuredCurrency
    : DEFAULT_USER_SETTINGS.currency;
  const configuredLanguage = String(configured.language || "").trim().toLowerCase();
  const safeLanguage = SUPPORTED_LANGUAGES.includes(configuredLanguage)
    ? configuredLanguage
    : DEFAULT_USER_SETTINGS.language;
  const configuredDateLocale = String(configured.dateLocale || "").trim();
  const safeDateLocale = SUPPORTED_DATE_LOCALES.includes(configuredDateLocale)
    ? configuredDateLocale
    : DEFAULT_USER_SETTINGS.dateLocale;
  const configuredTimezone = String(configured.timezone || "").trim();
  const safeTimezone = SUPPORTED_TIMEZONES.includes(configuredTimezone)
    ? configuredTimezone
    : DEFAULT_USER_SETTINGS.timezone;

  const privacy = isPlainObject(configured.privacy) ? configured.privacy : DEFAULT_PRIVACY_SETTINGS;
  const permissions = isPlainObject(privacy.permissions) ? privacy.permissions : DEFAULT_PRIVACY_SETTINGS.permissions;

  return {
    currency: safeCurrency,
    language: safeLanguage,
    dateLocale: safeDateLocale,
    timezone: safeTimezone,
    weekStart: safeWeekStart,
    defaultRange: isSupportedDateRange(configured.defaultRange)
      ? configured.defaultRange
      : DEFAULT_USER_SETTINGS.defaultRange,
    privacy: {
